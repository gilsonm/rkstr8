...
class SingleTaskLoop:

    def __init__(self, async_task, faild_task, succd_task, stats_path, pollr_wait_time):
        self.status_json_path = stats_path
        self.async_task_state = async_task
        self.poll_fail_state = faild_task
        self.poll_success_state = succd_task
    ...
    def states(self):
        ...
        waiter = Wait(
            name='-'.join(('Wait', self.uid)),
            wait_time=self.poll_wait_time,
            next=self.async_task_state.name
        )
        ...
        choice = Choice(
            ...
            Rule(
                ...
                next=self.poll_success_state.name
            ),
            Rule(
                StringEquals(
                    var=self.status_json_path,
        ...
        self.async_task_state.next = choice.name

        return [self.async_task_state, self.poll_success_state, self.poll_fail_state, waiter, choice]
    pass